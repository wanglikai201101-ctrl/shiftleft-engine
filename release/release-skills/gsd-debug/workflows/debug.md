...
## 3.5. Knowledge Graph Pre-lookup (optional, auto-skipped if unavailable)

Before delegating to the session manager, attempt to enrich the debug context with graph data.
This step is **silent-skip** if no graph or knowledge-base CLI is available.
...
if [ -n "$KB_CLI" ] && [ -f "graph/graph.json" -o -f "$KB_CLI/graph/graph.json" -o -f "modules/../graph/graph.json" ]; then
  # Extract file paths from error trace / symptoms
  # Run impact analysis on related nodes
  cd "$KB_CLI"
  PYTHONIOENCODING=utf-8 python -m packages.cli graph impact --node "<inferred-node>" --output graph 2>/dev/null || true
fi
```

**How to use the result:**
- If graph returns related nodes, append to the debug session file under a `## Graph Context` section:
  - Related requirements (why this code exists)
  - Related APIs/tables/pages (what else might be affected)
  - This gives the debugger agent immediate context without needing to explore from scratch

**If unavailable:** Skip silently. Never fail or warn — graph enrichment is a bonus, not a requirement.
