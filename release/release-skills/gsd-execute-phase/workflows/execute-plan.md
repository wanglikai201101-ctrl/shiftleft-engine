...
<step name="kb_conformance_check">
**Knowledge-base conformance gate (optional — runs only if tooling is available).**

Check whether the project has a knowledge-base conformance tool:

```bash
KB_CHECK_AVAILABLE="false"
if [ -f "knowledge-base/packages/cli/__main__.py" ]; then
  KB_CHECK_AVAILABLE="true"
fi
```

If `KB_CHECK_AVAILABLE` is `true`:

1. Run conformance check against changed files in this plan:
```bash
CHANGED_FILES=$(git diff --name-only HEAD~${TASK_COUNT}..HEAD 2>/dev/null | tr '\n' ' ')
python -m packages.cli check --all 2>/dev/null || true
```

2. Run graph orphan detection:
```bash
python -m packages.cli graph build --output graph 2>/dev/null || true
python -m packages.cli graph orphans --output graph 2>/dev/null || true
```

3. **Result handling:**
   - If check passes (exit 0) and no orphans: continue silently
   - If check fails or orphans detected: log findings in SUMMARY.md under `## Documentation Gaps` section — do NOT block execution
   - If commands fail (tool not installed, import error): skip silently — this is expected for projects without knowledge-base

This step is **advisory, not blocking** — it surfaces gaps for the next iteration without stopping the current execution.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `./templates/summary.md`.
