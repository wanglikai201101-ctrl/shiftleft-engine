...
- `--from-pipeline` flag → store `$PIPELINE_MODE=true` (signals this invocation comes from a pipeline enricher — do NOT scope-reduce)
- `--module <value>` flag → store `$MODULE=<value>` (consumed, removed from description)
- `--source <value>` flag → store `$SOURCE=<value>` (consumed, removed from description)
...
**Step 4.5: Knowledge Graph Pre-lookup (optional, auto-skipped if unavailable)**

Before planning, attempt to enrich context with graph data. Provides the planner with precise knowledge about what the change might impact.
...
if [ -n "$KB_CLI" ]; then
  GRAPH_DIR=""
  for candidate in "graph" "$KB_CLI/graph" "modules/../graph"; do
  ...
  if [ -n "$GRAPH_DIR" ]; then
    # Try to find relevant nodes from the task description keywords
    cd "$KB_CLI"
    GRAPH_CONTEXT=$(PYTHONIOENCODING=utf-8 python -m packages.cli graph stats --output "$GRAPH_DIR" 2>/dev/null || true)
  fi
fi
```

**If graph available:** Include graph stats in planner context. The planner can then use `/gsd-kb-query context <REQ-id>` to get precise info for specific requirements referenced in the task description.

**If unavailable:** Skip silently. Never fail or warn.
...
<constraints>
${PIPELINE_MODE ? '- This task comes from a pipeline enricher. The description is a COMPLETE specification.' : '- Create a SINGLE plan with 1-3 focused tasks'}
${PIPELINE_MODE ? '- Implement ALL sections listed in the description — do NOT scope-reduce or pick only one dimension.' : '- Quick tasks should be atomic and self-contained'}
${PIPELINE_MODE ? '- Create tasks covering every Desired Change section (typically 3-8 tasks for pipeline-sourced work).' : ''}
${PIPELINE_MODE ? '- Each task should map to one logical unit from the spec (e.g., one API endpoint, one UI component, one set of i18n keys).' : ''}
${RESEARCH_MODE ? '- Research findings are available — use them to inform library/pattern choices' : '- No research phase'}
${PIPELINE_MODE ? '- Target ~60% context usage (full spec implementation)' : (VALIDATE_MODE ? '- Target ~40% context usage (structured for verification)' : '- Target ~30% context usage (simple, focused)')}
