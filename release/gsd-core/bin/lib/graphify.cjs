...
/**
 * Load synonyms.json from disk. Cached after first load per path.
 * Falls back gracefully to empty map on missing/malformed file.
 */
function loadSynonyms(synonymsPath) {
    if (!synonymsPath)
        return {};
    if (_synonymCache && _synonymCache.path === synonymsPath)
        return _synonymCache.data;
    try {
        if (!node_fs_1.default.existsSync(synonymsPath))
            return {};
        const raw = JSON.parse(node_fs_1.default.readFileSync(synonymsPath, 'utf8'));
        const data = raw?.aliases || {};
        _synonymCache = { path: synonymsPath, data };
        return data;
    }
    catch {
        return {};
...
/**
 * Expand a search term to include all synonyms from the alias map.
 */
function expandSynonyms(term, synonyms) {
    const terms = [term];
    const termLower = term.toLowerCase();
    for (const [key, aliases] of Object.entries(synonyms)) {
        const allVariants = [key.toLowerCase(), ...aliases.map(a => a.toLowerCase())];
        if (allVariants.includes(termLower)) {
...
/**
 * Detect query intent from term format.
 * Returns true if term looks like an exact node ID (module:type:slug pattern).
 */
function isExactNodeId(term, graph) {
    // Check if it matches the module:type:slug convention
    if (/^[a-z_-]+:[a-z]+:/.test(term)) {
        const nodeMap = Object.fromEntries((graph.nodes || []).map(n => [n.id, n]));
        return Boolean(nodeMap[term]);
    }
    return false;
}
/**
 * Compute relevance score for a node against a set of search terms.
 * Scoring: exact_label=10, prefix_label=5, substr_label=3, exact_id=8, substr_id=2
 */
function computeNodeScore(node, terms) {
    let score = 0;
    ...
    for (const t of terms) {
        if (t === labelLower)
            ...
            score += 5;
        else if (labelLower.includes(t))
            score += 3;
        if (t === idLower)
            score += 8;
        else if (idLower.includes(t))
            score += 2;
...
/**
 * Resolve synonyms.json path for a project. Checks:
 * 1. .planning/graphs/synonyms.json (project override)
 * 2. knowledge-base/packages/core/graph/synonyms.json (default, relative to cwd)
 */
function resolveSynonymsPath(cwd) {
    const projectOverride = node_path_1.default.join(cwd, '.planning', 'graphs', 'synonyms.json');
    if (node_fs_1.default.existsSync(projectOverride))
        return projectOverride;
    // Walk up from cwd looking for knowledge-base/packages/core/graph/synonyms.json
    const kbPath = node_path_1.default.join(cwd, 'knowledge-base', 'packages', 'core', 'graph', 'synonyms.json');
    if (node_fs_1.default.existsSync(kbPath))
        return kbPath;
    return null;
}
let _searchIndexCache = null;
/**
 * Load search-index.json (BM25 inverted index). Cached after first load.
 */
function loadSearchIndex(cwd) {
    const indexPath = node_path_1.default.join(cwd, '.planning', 'graphs', 'search-index.json');
    const kbPath = node_path_1.default.join(cwd, 'knowledge-base', 'graph', 'search-index.json');
    const resolvedPath = node_fs_1.default.existsSync(indexPath) ? indexPath : (node_fs_1.default.existsSync(kbPath) ? kbPath : null);
    if (!resolvedPath)
        return null;
    if (_searchIndexCache && _searchIndexCache.path === resolvedPath)
        return _searchIndexCache.data;
    try {
        const raw = JSON.parse(node_fs_1.default.readFileSync(resolvedPath, 'utf8'));
        const data = { terms: raw?.terms || {} };
        _searchIndexCache = { path: resolvedPath, data };
        return data;
    }
    catch {
        return null;
...
/**
 * Tokenize a query term for search index lookup (mirrors Python _tokenize).
 */
function tokenizeQuery(text) {
    const tokens = [];
    const parts = text.toLowerCase().split(/[^a-zA-Z0-9一-鿿]+/);
...
/**
 * Score seeds using the BM25 inverted index (fast O(1) path).
 * Returns node IDs sorted by score, limited to top-K.
 */
function scoreViaSearchIndex(terms, searchIndex, topK) {
    const nodeScores = {};
    // Tokenize all expanded terms for index lookup
    const queryTokens = new Set();
    for (const t of terms) {
        queryTokens.add(t);
        for (const tok of tokenizeQuery(t)) {
            queryTokens.add(tok);
        }
    }
    for (const token of queryTokens) {
        const postings = searchIndex.terms[token];
        if (!postings)
            continue;
        for (const posting of postings) {
            nodeScores[posting.id] = (nodeScores[posting.id] || 0) + posting.score;
        }
    }
 ...
 * Enhanced with synonym expansion, intent detection, and score-based seed ranking.
 */
function seedAndExpand(graph, term, maxHops = 2, options) {
    const nodeMap = Object.fromEntries((graph.nodes || []).map(n => [n.id, n]));
    const adj = buildAdjacencyMap(graph);
    // Intent detection: exact node ID → BFS directly from that node
    if (isExactNodeId(term, graph)) {
        const seedNode = nodeMap[term];
        if (!seedNode)
            return { nodes: [], edges: [], seeds: new Set() };
        const visitedNodes = new Set([term]);
        ...
        return { nodes: resultNodes, edges: collectedEdges, seeds: new Set([term]) };
    }
    // Load synonyms & expand terms
    const synonyms = loadSynonyms(options?.synonymsPath);
    const terms = expandSynonyms(term, synonyms);
    // Build combined term set with multi-word splitting
    const allTerms = new Set();
    for (const t of terms) {
        allTerms.add(t.toLowerCase());
        for (const part of t.split(/[\s\-_]+/)) {
            if (part.length >= 2)
                allTerms.add(part.toLowerCase());
        }
    }
    // Fast path: use BM25 inverted index if available
    let seeds;
    const searchIndex = options?.searchIndex;
    if (searchIndex && Object.keys(searchIndex.terms).length > 0) {
        const topScored = scoreViaSearchIndex(allTerms, searchIndex, 10);
        seeds = topScored.map(s => nodeMap[s.id]).filter((n) => Boolean(n));
    }
    else {
        // Fallback: score-based seed selection (linear scan)
        const scored = (graph.nodes || []).map(n => ({
            node: n,
            score: computeNodeScore(n, allTerms)
        })).filter(s => s.score > 0)
            .sort((a, b) => b.score - a.score)
            .slice(0, 10);
...
function graphifyQuery(cwd, term, options = {}) {
    ...
    const synonymsPath = resolveSynonymsPath(cwd);
    const searchIndex = loadSearchIndex(cwd);
    let result = seedAndExpand(graph, term, 2, { synonymsPath, searchIndex });
    if (options.budget) {
...
module.exports = {
    ...
    seedAndExpand,
    applyBudget,
    // Search enhancement helpers (Phase 1 — synonym/scoring)
    loadSynonyms,
    expandSynonyms,
    computeNodeScore,
    isExactNodeId,
    resolveSynonymsPath,
    // Search enhancement (Phase 2 — inverted index)
    loadSearchIndex,
    scoreViaSearchIndex,
    tokenizeQuery,
    // Status (Phase 2)
