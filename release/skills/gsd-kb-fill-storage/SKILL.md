...
1. 🚫 NEVER write files outside `$OUTPUT/$MODULE/storage/` — 禁止写入 apis/, pages/, tech/, graph/, tests/, requirements/, jobs/
2. 🚫 NEVER create files with non-standard names — only `{table-name}.md` format (lowercase, hyphen-separated)
   - ✅ Allowed: `agents.md`, `sandbox-instances.md`, `agent-versions.md`
   - ❌ Forbidden: `SERVICE-*.md`, `_INDEX.md`, `SUMMARY.md`, `README.md`
3. 🚫 NEVER document non-storage concerns — this skill documents DATABASE TABLES only
   - ❌ Forbidden: service class docs, API endpoint docs, page docs
   - ✅ Allowed: table schema, field types, constraints, read/write traceability
...
只有当 `--source` 内没有找到任何 model 时，才 fallback 到 PROJECT_ROOT，但 fallback 时必须用 module 名做路径过滤。

### Scope definition

```bash
# PRIMARY scope: --source 路径（模块聚焦）
SCAN_DIR="$SOURCE"

# FALLBACK scope: 只在 PRIMARY 内找不到任何 table 时使用
# 且必须过滤为只含 module 名的路径
FALLBACK_DIR="$PROJECT_ROOT"
FALLBACK_FILTER="*${MODULE}*"
```

### Strategy 1: ORM models
If `--models-dir` passed, use it directly. Otherwise:
- **First:** search within `$SCAN_DIR` for files with `__tablename__`
  ```bash
  grep -rl "__tablename__" "$SCAN_DIR" --include="*.py" | grep -v .venv | grep -v node_modules
  ```
- **Only if 0 results:** expand to `$FALLBACK_DIR` with module filter
  ```bash
  grep -rl "__tablename__" "$FALLBACK_DIR" --include="*.py" | grep -v .venv | grep -v node_modules | grep -i "$MODULE"
  ```
...
```bash
# Primary: within --source
grep -rl "CREATE TABLE" "$SCAN_DIR" --include="*.py" --include="*.sql" | grep -v .venv | grep -v node_modules

# Fallback: PROJECT_ROOT with module filter (ONLY if primary returns 0)
grep -rl "CREATE TABLE" "$FALLBACK_DIR" --include="*.py" --include="*.sql" | grep -v .venv | grep -v node_modules | grep -i "$MODULE"
```
...
🚫 This strategy is already module-scoped (reads from $MODULE/apis/). Do not expand scope.

### Merge and deduplicate
Combine all three sources into a unique table list.

### Module boundary validation (🔒 mandatory post-merge)

After merge, validate each discovered table belongs to this module:

```bash
# For each table, check if its source file is within $SOURCE or contains $MODULE in path
for table_source in $DISCOVERED_SOURCES; do
  if [[ "$table_source" != *"$MODULE"* ]] && [[ "$table_source" != "$SCAN_DIR"* ]]; then
    echo "⚠️ EXCLUDED: $table_source (outside module boundary)"
    # Remove from table list
  fi
done
```

Report excluded tables:
```
Module boundary filter:
  Tables discovered (raw): {N}
  Tables excluded (outside module): {N} — {list}
  Tables retained: {N}
```
...
**KB CLI resolution (canonical — do NOT search $HOME):**
```bash
...
```bash
cd "$KB_CLI"
# 🔒 scaffold 必须用 $SOURCE（模块级），不得用 $PROJECT_ROOT
# 🔒 batch-fill 的 --source 也必须用 $SOURCE，限定代码搜索范围
PYTHONIOENCODING=utf-8 python -m packages.cli scaffold --source "$SOURCE" --module "$MODULE" --output "$OUTPUT" --no-auto-detect
PYTHONIOENCODING=utf-8 python -m packages.cli --kb-path "$OUTPUT" batch-fill --module "$MODULE" --source "$SOURCE" --workers 8
```

🚫 NEVER pass `$PROJECT_ROOT` or `$MODELS_DIR` to scaffold/batch-fill — 这会导致跨模块扫描。
🚫 `$SOURCE` 就是用户传入的 `--source` 参数值（已限定到模块级目录）。

**Post-CLI validation (🔒 mandatory):**

CLI 执行完后，必须用 Step 2 的 Module boundary validation 验证 CLI 产出：
```bash
# 检查 CLI 生成的 storage 文档是否有越界
for doc in "$OUTPUT/$MODULE/storage/"*.md; do
  source_ref=$(grep -m1 "来源:" "$doc" | sed 's/.*`\(.*\)`.*/\1/')
  if [[ -n "$source_ref" ]] && [[ "$source_ref" != *"$MODULE"* ]] && [[ "$source_ref" != "$SOURCE"* ]]; then
    echo "⚠️ CLI BOUNDARY VIOLATION: $doc references $source_ref"
    # 删除越界文档
    rm "$doc"
    echo "   REMOVED: $doc (outside module boundary)"
  fi
done
```
...
**如果目标 storage 文档已存在且符合模板规范**（所有必需 `##` 段齐全，关键字段无 `待补充`）：
