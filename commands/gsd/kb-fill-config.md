...
- `--force` (optional): re-generate ALL config docs, overwriting existing content. Without --force, only create new config docs for undocumented configs and skip existing ones.
...
<notes>
- Safe to re-run: skips existing config doc files (unless --force is passed)
