...
- `/gsd-kb-gen-tests-api` → mcp_run_test_chain format
- `/gsd-kb-gen-tests-e2e` → mcp_run_e2e_scenario format
- `/gsd-kb-gen-tests-ui` → submit_test format
...
```json
{
  ...
  "notes": "QA Center handles login internally. Session config sets variables for API test chains."
}
```
...
- api: API contract tests (mcp_run_test_chain) — fast, no browser
- e2e: E2E scenario tests (mcp_run_e2e_scenario) — multi-step flows
- ui: UI automation tests (submit_test) — browser-based, agent-driven
- all: Generate all basic types (api + e2e + ui)
- perf: Performance baseline tests (mcp_run_concurrent_tests) — 专项，需显式指定
...
```json
{
  "_meta": {"type": "chain", "mcp_tool": "mcp_run_e2e_scenario"},
...
```json
{
  "_meta": {
    "type": "perf",
    "mcp_tool": "mcp_run_concurrent_tests",
...
```json
{
  ...
  "execution_guide": {
    "api_regression": {
      "tool": "mcp_run_test_chain",
      "directory": "tests/api/",
      "estimated_time": "2-5 minutes"
    },
    "e2e_flows": {
      "tool": "mcp_run_e2e_scenario",
      "directory": "tests/e2e/",
...
Execute via MCP:
  API:  mcp_run_test_chain(steps=<file>.steps)
  E2E:  mcp_run_e2e_scenario(name=<file>.name, steps=<file>.steps)
  UI:   submit_test(name, url, steps, expected_results, options)
  Perf: mcp_run_concurrent_tests(url, method, headers, concurrency, timeout)
```
...
- Validation: each sub-skill runs post-generation validation (dry-run for API/E2E, static for UI)
- Dry-run validation uses actual API responses as source of truth over KB documentation
- Validation status is recorded in each test file's _meta field
</notes>
